    ...
    def train(self, input_=None, target=None, show=10):
        ...
        for ep in range(self.maxEpoch):
            ...
            if epoch_error < self.thresholdError:
                print()
                "Finish {0}: ".format(ep), epoch_error
                return
            elif ep % show == 0:
                print()
                "epoch {0}: ".format(ep), epoch_error
...
if __name__ == "__main__":
    ...
    Xtr, Ytr = train_x[:5000],train_y[:5000]
    print(Ytr.shape)

    Xte, Yte = test_x[:5000],test_y[:5000]  # 5000 for training (nn candidates)
    ...
    print()
    "training error: ", sum(np.array(sum(0.5 * np.multiply(train_labels - np.mat(np.array(sims).transpose()),
    ...
    print()
    "test error: ", sum(np.array(sum(0.5 * np.multiply(test_labels - np.mat(np.array(sims_test).transpose()),
